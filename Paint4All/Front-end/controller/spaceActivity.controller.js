...
// funcao de gravaçao
function saveActividadesEspaco() 
{ 
    ...
    fetch('localhost:8080/SpaceActivities/', 
    { 
        //cabecalho
        headers: 
        {
...
function refreshActividadesEspaco() {
    async function fetchAsync() {
    const renderActividadesEspaco = document.getElementById("result");
    let txt = "";
    const response = await fetch('localhost:8080/SpaceActivities/',);
    const ActividadesEspaco = await response.json();
    ...
    //percorrer a variável ActividadesEspaco e por cada ActividadeEspaco cria a linha da tabela com os dados presentes
    for (const ActividadeEspaco of ActividadesEspaco) {
    ...
    renderActividadesEspaco.innerHTML = txt;
    }
