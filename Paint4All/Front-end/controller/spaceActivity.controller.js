const server = 'http://localhost:8080/spaceActivities/';
window.onload = function () 
{
    alert("pagina carregada com sucesso!!")
    refreshSpaceActivities();
    validator();
...
function refreshSpaceActivities() {
    async function fetchAsync() {
    const renderSpaceActivities= document.getElementById("result");
    let txt = "";
    const response = await fetch(server,);
    const spaceActivities = await response.json();
    ...
    for (const spaceActivity of spaceActivities) {
    ...
    renderSpaceActivities.innerHTML = txt;
    }
